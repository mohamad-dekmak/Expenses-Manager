...
public class ExpensesController implements Serializable {
    ...
    @EJB
    private m.dekmak.ExpensesFacade ejbFacade;
    ...
    public void create() {
        persist(PersistAction.CREATE, ResourceBundle.getBundle("/BundleExpense").getString("ExpensesCreated"));
    ...
    public void update() {
        persist(PersistAction.UPDATE, ResourceBundle.getBundle("/BundleExpense").getString("ExpensesUpdated"));
    }

    public void destroy() {
        persist(PersistAction.DELETE, ResourceBundle.getBundle("/BundleExpense").getString("ExpensesDeleted"));
        if (!JsfUtil.isValidationFailed()) {
        ...
        if (selected != null) {
            ...
            try {
                ...
                if (msg.length() > 0) {
                    ...
                    JsfUtil.addErrorMessage(ex, ResourceBundle.getBundle("/BundleExpense").getString("PersistenceErrorOccured"));
                }
            } catch (Exception ex) {
